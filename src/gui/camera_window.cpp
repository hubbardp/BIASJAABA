...
namespace bias
{
    ...
    const QString DEFAULT_TESTCONFIG_FILE_NAME = QString("bias_testconfig");
    ...
    RtnStatus CameraWindow::startImageCapture(bool showErrorDlg) 
    {
        ...
        if (trigEnumVal == TRIGGER_EXTERNAL && trigExternalEnumVal == TRIGGER_NIDAQ) {
            ...
            nidaq_task->releaseLock();*/
#else
            // making sure nidaq pointer is not null before proceeding forward
...
#if debugNIDAQ
                timerClass_->nidaqTimerptr = nidaq_task;
                currentPluginPtr->setupTimerForPlugin(timerClass_, loadTestConfigEnabled,
                   trial_num, testConfig);
#else

                currentPluginPtr->setupTimerForPlugin(timerClass_,
        ...
        //connect to cameraWindowptr to signal gpu deInitalize
        connect(imageGrabberPtr_,
            SIGNAL(signalGpuDeinit()),
            this,
            SLOT(gpuDeInitialize()));
    ...
    RtnStatus CameraWindow::startProcessThreads(bool showErrorDlg)
    {
        ...
        if (isPluginEnabled()) {
            ...
            QPointer<BiasPlugin> currentPluginPtr = getCurrentPlugin();
            //start jaaba process scores thread
            if (cmdlineparams_.classify_scores && !isPluginStarted)
            {      
                currentPluginPtr->reset();
            }
    ...
    RtnStatus CameraWindow::stopImageCapture(bool showErrorDlg)
    {
        ...
        if (isPluginEnabled() && cmdlineparams_.classify_scores)
        {
        ...
        if (isPluginEnabled())
        {
            ...
            if (!currentPluginPtr.isNull())
            {
                ...
                std::cout << "plugin  stop function called " << std::endl;
            }
        }
        ...
        if (timerClass_->nidaqTimerptr != nullptr)
        {
            timerClass_->nidaqTimerptr->clearTimerTasks();
        }
#endif
    ...
    RtnStatus CameraWindow::startTrigger(bool showErrorDlg)
    {
        ...
        if (nidaq_task != nullptr && 
            !nidaq_task->isTaskStarted() 
            && !nidaq_task->isTriggered())
        {
...
#if debugNIDAQ 
        if (nidaq_task != nullptr && nidaq_task->isTriggered())
        {
            // set stopTrigger flag in imagegrab for all cameras
            setStopNIDAQTriggerFlag();
            nidaq_task->stopTimerTrigger();
            nidaq_task->stopTimerTasks();
    ...
    RtnStatus CameraWindow::loadTestConfiguration(QString fileName, bool showErrorDlg)
    {
    ...
    /*void CameraWindow::setTriggered(bool nidaqTriggered)
    {
        std::cout << "Here" << std::endl;
        QPointer<CameraWindow>partnerCameraWindowPtr = getPartnerCameraWindowPtr();
        QPointer<ImageGrabber>partnerImageGrabberPtr = partnerCameraWindowPtr->getImageGrabberPtr();
        partnerImageGrabberPtr->nidaqTriggered = nidaqTriggered;
    }*/
    ...
    void CameraWindow::actionTestLoadConfigTriggered()
    {

        std::cout << "Load Test Configuration " << std::endl;
        QString configFileFullPath = getTestConfigFileFullPath();
    ...
    void CameraWindow::gpuDeInitialize()
    {
    ...
    QString CameraWindow::getTestConfigFileFullPath()
    {
        ...
        switch (triggerType)
        {
            ...
            case TRIGGER_EXTERNAL:
                ...
                if (triggerExternalType == TRIGGER_NIDAQ) {
                    ...
                    timerClass_->cameraMode = true;
#if debugNIDAQ
                    //setup nidaq config from map
                    if (nidaq_task != nullptr && cameraNumber_ == 0)
                    {
                        if (!triggerExternalTypeMap.isEmpty()) {
                            rtnStatus = nidaq_task->configureTimer(cameraNumber_,triggerExternalTypeConfigMap);
                            if (!rtnStatus.success) {
                    ...
                    if (cameraNumber_ == 0) {
                        ...
                        timerClass_->nidaqTimerptr = make_shared<Lockable<NIDAQUtils>>();

                        std::cout << "NIDAQ allocated in cameranumber " << cameraNumber_ << std::endl;

                        //allocate timers
                        rtnStatus = timerClass_->nidaqTimerptr->configureTimer(cameraNumber_, triggerExternalTypeConfigMap);
                        if (!rtnStatus.success)
                        {
    ...
    void CameraWindow::connectSignals()
    {
        ...
        connect(this,
            SIGNAL(threadImageGrabStopped()),
            partnerCameraWindowPtr,
            SLOT(stopImageGrabThreads())
        );
    }
    ...
    void CameraWindow::setTrialNum()
    {
        if(!imageGrabberPtr_.isNull())
            imageGrabberPtr_->setTrialNum(trial_num);
        ...
        if (isPluginEnabled())
        {
            ...
            {
                if (currentPluginPtr->getName() == "jaabaPlugin") {
                    currentPluginPtr->setTrialNum(trial_num);
                }
            }
        }
    }

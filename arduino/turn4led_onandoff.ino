...
unsigned long led_on_interval = 50UL; // time in ms
int isSequence = 0;
int current_pin_id = 1;
...
void convertSerialtoBinary(char serial_input) {
  ...
  for (byte led_id = 0; led_id < numSignaling_leds; led_id++) {
    byte state = bitRead(serial_input, led_id);
    if(isSequence){
      digitalWrite(output_led_state[led_id], state);
    }
    else if (current_pin_id == led_id){
      digitalWrite(output_led_state[led_id], state);
    }
...
void setup() {
  ...
  if (isSequence)
    ledTimer.begin(singleLedOnAndOff, 400000); // turn on sequence every 0.40 secs
  else
...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    if (serial_trig > '0' && serial_trig < '9') {
      convertSerialtoBinary(serial_trig);
      //serial_trig = '0';
    } else if (serial_trig == '0')
    {
      digitalWrite(DIGITAL5, LOW);
      digitalWrite(DIGITAL6, LOW);
      digitalWrite(DIGITAL7, LOW);
